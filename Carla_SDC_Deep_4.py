...
MODEL_NAME = "Xception"
MEMORY_FRACTION = 0.6 #how much gpu you wanna use
MIN_REWARD = -200
EPISODES = 100
DISCOUNT = 0.99
epsilon = 1
...
#gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=MEMORY_FRACTION) #TENSORFLOW DON'T USE ALL MY GPU PLZ
...
class DQNAgent:
    ...
    def __init__(self):
        ...
        #Target model this is what we . against every step
...
if __name__ == "__main__":
    ...
    #Target model is training with train_in_loop
    ...
    # tqdm : Instantly make your loops show a smart progress meter - just wrap any iterable with tqdm(iterable) , and you're done! from
        env.collision_hist = []
        agent.tensordboard.step = episode 
        episode_reward=0
        step=1
        current_state = env.reset() #first obs ever
        done = False
        episode_start = time.time()

        #While True or "while not done"
        while True:
